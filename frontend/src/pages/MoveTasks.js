      ...
      if (editItem) {
        const payload = {
          ...
          planned_start_time: formData.planned_start_time
            ? new Date(formData.planned_start_time).toISOString()
            : null,
          planned_end_time: formData.planned_end_time
            ? new Date(formData.planned_end_time).toISOString()
            : null,
          ...
          move_plan_id: formData.move_plan_id === '' ? null : formData.move_plan_id,
          status: formData.status,
        };
        await updateMoveTask(editItem.id, payload);
        ...
        <DialogContent>
          <Box sx={{ pt: 1, display: 'flex', flexDirection: 'column', gap: 2 }}>
            ...
            <TextField
              ...
              onChange={(e) => setFormData({ ...formData, move_plan_id: e.target.value === '' ? '' : parseInt(e.target.value) })}
              fullWidth
            >
